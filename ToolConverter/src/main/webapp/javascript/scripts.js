...
function renderFileList() {
  ...
  files.forEach((file) => {
    ...
    const convertToXmlButton = createConvertButton(file, "XML");
    const convertToJsonButton = createConvertButton(file, "JSON");

    fileActionButtons.appendChild(convertToXmlButton);
    fileActionButtons.appendChild(convertToJsonButton);
...
function createConvertButton(file, format) {
  const convertButton = document.createElement("button");
  convertButton.textContent = format;
...
function convertFileToJson(file) {
  ...
  fetch("convertToJson.action", {
    method: "POST",
    body: formData,
  })
    .then((response) => {
      if (!response.ok) {
        throw new Error("Errore durante la conversione del file in JSON.");
      }
      return response.blob(); // restituisce un oggetto Blob
    })
    .then((blob) => {
  ...
  addToActivityList(`File converted to JSON: ${file.name}`);
}
